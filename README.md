...
The customer–customer network decomposes into 25 connected components, meaning the graph consists of 25 disconnected sub-networks with no paths between them. Component sizes are relatively similar, ranging from 124 nodes (3.2%) to 171 nodes (4.4%), indicating there is no dominant “giant component.” Fragmentation is very high (normalized heterogeneity = 0.96 in an undirected graph), which suggests the overall network is highly disconnected under the current tie definition. Given this structure, network measures are most interpretable when computed within components (e.g., focusing on the largest component(s)) rather than across the full graph.
